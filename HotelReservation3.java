...
public class HotelReservation3 extends JFrame implements ActionListener{
  ...
  public void actionPerformed (ActionEvent e){
    ...
    if (e.getSource() == submitButton) {
      ...
      //Output Box
      String message = "Your reservation has been successfully submitted" + 
      ...
      try{
        ...
        //Connect to server to verify CC
        String verified = "";
        try {
          ...
          output.writeObject(creditCardNum + creditCardExp);
          output.flush();
          
          socket = serverSocket.accept();
          input = new ObjectInputStream(socket.getInputStream());
          verified = (String) input.readObject();
        ...
        if (verified.equalsIgnoreCase("Yes")){
          writeAcctReservation (creditCardNum, message);
          int gPerRoom = Integer.parseInt(guestsRoom);
          saveReservationToDatabase (firstName, lastName, checkInDate, checkOutDate, numRooms, gPerRoom, specialRates, 
                                     studio, standard, family, suite, specialRequests, street, city, state, zipCode, 
                                     phoneNumber, creditCardType, creditCardNum, creditCardExp);
          
          JOptionPane.showMessageDialog(null, message);
        }
        else{
          JOptionPane.showMessageDialog(null, "Your reservation cannot be made due to invalid credit card");
        }
      
      }
      
      catch (InvalidCreditCardException cce){ JOptionPane.showMessageDialog(null, cce.getMessage()); }
  ...
  private void saveReservationToDatabase(String firstName, String lastName, String checkIn, String checkOut, 
                                         String numRooms, int gPerRoom, String specialRate, boolean studio, 
                                         boolean standard, boolean family, boolean suite, String specialRequest,
                                         String street, String city, String state, String zip, String phone, 
                                         String creditCard, String creditCardNum, String expDate){
    
    String url = "jdbc:mysql://localhost/hotel?user=root&pass=";
    
    String stud, stan, fami, suit;
    if (studio){
      stud = "Y";
    }
    else{
    ...
    try{
      ...
      String insertStatement = "insert into reservation values('" +firstName + "', '" + lastName + "', '" + checkIn 
        + "', '" + checkOut + "', '" + numRooms + "', " + gPerRoom + ", '" + specialRate + "', '" + stud + "', '" 
        + stan + "', '" + fami + "', '" + suit + "', '" + specialRequest + "', '" + street + "', '" +  city + "', '" + 
        state + "', '" + zip + "', '" + phone + "', '" + creditCard + "', '" + creditCardNum +  "', '" + expDate + "');";
  ...
  public static void main (String [] args){
    ...
    HotelReservation3 app = new HotelReservation3();
    
  }//end main method
  
}//end HotelReservation3